...
## Filename:       start-workers.sh
## Desctiption:    启动所有的worker
## Author:         chenke
## Created:        2018-09-17
###################################################################
...
function start_workers()
{
     ...
     echo "**********************************" | tee -a ${PEOMAN_LOG_FILE}
     echo "" | tee -a ${PEOMAN_LOG_FILE}
     echo "开始执行启动每个节点上的worker......" | tee -a ${PEOMAN_LOG_FILE}
     ...
     do
       ssh root@${hostname} "sh ${WORKER_BIN}/start-worker.sh"
       echo "${hostname} 上启动worker完毕.............." | tee -a ${PEOMAN_LOG_FILE}
     done
}
