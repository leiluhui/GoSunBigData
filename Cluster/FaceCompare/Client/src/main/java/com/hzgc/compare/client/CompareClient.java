...
public class CompareClient {
    ...
    public SearchResult retrievalOnePerson(CompareParam paramt){
        ...
        List<Integer> sorts = paramt.getSort();
        if(sorts != null && (sorts.size() > 1 || (sorts.size() == 1 && sorts.get(0) != 4))){
            ...
            for(int i = 1 ; i < list.size(); i ++){
                result.merge(list.get(i), sortParams);
            }
            return result;
        }
        List<SearchResult> list = searchRes.getResult();
        ...
        if(sorts != null && (sorts.size() > 1 || (sorts.size() == 1 && sorts.get(0) != 4))){
            ...
            for(int i = 1 ; i < list.size(); i ++){
                ...
                for(Map.Entry<String, SearchResult> entry : result.entrySet()){
                    entry.getValue().merge(res.get(entry.getKey()), sortParams);
                }
            }
            return result;
        }
    ...
    public static void main(String args[]) throws InterruptedException, UnsupportedEncodingException {
        CompareClient client = new CompareClient();
        client.createService("172.18.18.103:2181");
