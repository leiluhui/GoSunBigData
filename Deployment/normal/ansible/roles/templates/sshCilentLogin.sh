...
## 配置文件目录
export CONF_DIR=${ROOT_HOME}
## 安装日记目录
#export LOG_DIR=${ROOT_HOME}/logs
## 安装日记目录
#export LOG_FILE=${LOG_DIR}/no_password.log
## 系统root 用密码
export PASSWORD=$(grep ansible_ssh_pass ${CONF_DIR}/hosts | cut -d '=' -f3)
## authorized_keys 内容所在文件
export AUTHORIZED_KEYS=${BIN_DIR}/authorized_keys.log

## 集群所有节点主机名，放入数组中
CLUSTER_HOSTNAME_LISTS=$(grep ansible_ssh_pass ${CONF_DIR}/hosts | cut -d ' ' -f1)
