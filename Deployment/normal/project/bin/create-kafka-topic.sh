...
function create_kafka_topic()
{
   ...
   #创建kafka imsi topic
    ...
    --replication-factor ${repl_factor} \
    --partitions ${part_num} \
    --topic alarm_message >> ${LOG_FILE} 2>&1 &
    if [ $? = 0 ]; then
        echo "创建 alarm_message topic 成功...."  | tee -a $LOG_FILE
        echo "kafka alarm_message topic 副本数为${repl_factor},分区数为${part_num}." | tee -a $LOG_FILE
    else
