...
DYNCAR_JAR_NAME=`ls ${LIB_DIR} | grep ^dyncar-[0-9].[0-9].[0-9].jar$`
DYNCAR_JAR=${LIB_DIR}/${DYNCAR_JAR_NAME}
...
{
   DYNCAR_PID=`jps | grep ${DYNCAR_JAR_NAME} | awk '{print $1}'`
   if [ -n "${DYNCAR_PID}" ];then
       echo "dyncar service already started"
   else
       nohup java -jar ${DYNCAR_JAR} --spring.profiles.active=pro  \
       --eureka.ip=${EUREKA_IP} \
